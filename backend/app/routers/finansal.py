...
@router.put("/leasing-odemeleri/{odeme_id}/ode", response_model=LeasingOdemeYanit,
            dependencies=[Depends(izin_gerektir("LEASING_DUZENLE"))])
def leasing_odeme_yap(odeme_id: int, istek: OdemeTahsilIstegi, db: Session = Depends(get_db)):
...
@router.put("/taksit-detay/{taksit_id}/tahsil-et", response_model=TaksitDetayYanit,
            dependencies=[Depends(izin_gerektir("TAKSIT_DUZENLE"))])
def taksit_tahsil_et(taksit_id: int, istek: TaksitTahsilIstegi, db: Session = Depends(get_db)):
...
@router.put("/kiralama-odemeleri/{odeme_id}/tahsil-et", response_model=KiralamaOdemeYanit,
            dependencies=[Depends(izin_gerektir("KIRALAMA_DUZENLE"))])
def kiralama_odemesi_tahsil_et(odeme_id: int, istek: OdemeTahsilIstegi, db: Session = Depends(get_db)):
...
def bakim_kaydi_olustur(istek: BakimOlusturIstegi, sirket_id: int = Depends(aktif_sirket_id_getir), db: Session = Depends(get_db)):
    yeni = BakimKaydi(sirket_id=sirket_id, **istek.model_dump())
    db.add(yeni)
    db.commit()
    db.refresh(yeni)
    return yeni
